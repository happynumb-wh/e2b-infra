...
# ==================== E2B DEBUG: 第二层 systemd 失败排查 ====================
# 注意:整个脚本是 set -eu,诊断命令都要 || true 容错,且不引用可能不存在的路径
{
echo "===E2B-DBG id-root=== $(id)"
echo "===E2B-DBG umask=== $(umask)"
echo "===E2B-DBG _apt-user=== $(id _apt 2>&1)"
echo "===E2B-DBG systemd-network-user=== $(id systemd-network 2>&1)"
echo "===E2B-DBG mounts==="
cat /proc/mounts 2>&1 | grep -E " / |/var|/run|/tmp" | head
