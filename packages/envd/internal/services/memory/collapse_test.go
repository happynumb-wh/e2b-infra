...
func TestAnonRWRegions(t *testing.T) {
	...
	// arenas), plus a file-backed mapping, a [stack] pseudo-mapping, a read-only
	// anon mapping, and a zero-length region — all of which must be excluded.
	maps := []*procfs.ProcMap{
		...
		{StartAddr: 0x014c00000000, EndAddr: 0x014c00000000, Perms: rw()},
